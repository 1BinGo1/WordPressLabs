<?php
    ...
    //error_reporting(0);
    require_once get_template_directory() . '/lab4/phpquery-master/phpQuery/phpQuery.php';
    $links = array();
    ...
    if (!empty($_POST['link'])){
        ...
        $query = $doc->find('head link[rel="stylesheet"]');
        ...
        $query = $doc->find('body link[rel="stylesheet"]');
        foreach ($query as $item){
            $tl = pq($item)->attr('href');
            if ($tl !== null && $tl !== "" && $tl !== "#" && $tl !== "/"){
                $links['body']['links'][pq($item)->attr('href')] = trim(pq($item)->text());
        ...
        function editUrl($key){
            ...
            if (mb_substr($key,0,3) == '../'){
                $mas = parse_url($_POST['link']);
                $path = $mas["scheme"] . "://" . $mas['host'] . substr($key,3);
            }
        }
                    ...
                    <li><a href="#">Scripts</a>
                        <ul>
                            <?php foreach ($links['body']['scripts'] as $key => $value): ?>
                                <?php editUrl($key); ?>
                                <li><p><a href="<?= $key; ?>" target="_blank"><?= $key; ?></a></p></li>
                            <?php endforeach; ?>
                        </ul>
                    </li>
                    <li><a href="#">Content</a>
